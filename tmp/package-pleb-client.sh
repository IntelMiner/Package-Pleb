...
#Let's set some variables first, too
WGET=http://patrician.intelminer.com

exec > log-output.txt
mkdir /tmp/updates
cd /tmp/updates/
wget -N $WGET/package-pleb/toolset.tar.gz
...
/tmp/updates/wget -N $WGET/package-pleb/update-package-pleb.sh
...
wget -N $WGET/package-pleb/package-list.sh
wget -N $WGET/package-pleb/sha1-list.txt
